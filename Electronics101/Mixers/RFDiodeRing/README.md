...
includes a complete evolution of the diode ring mixer.

See [LEAP#326](../) for my notes on mixer theory and types.

### How it works

* transformers provide isolation of 1st order products
* the RF input is connected to the IF via a transformer
* the LO switches the conducting diode pairs on each each half of it's cycle, thus the polarity if the RF signal to IF
...
This works pretty well, alough my signals are very noisy.
...
I'm using 1N5711 small signal schottky diodes (as did w2aew), which should be well suited to this application:
