...
void Sparklie::clear(lr::AS1130Picture24x5& page) {
  page.setPixel(WIDTH - x, y, false);
}


void Sparklie::draw(lr::AS1130Picture24x5& page, lr::AS1130& driver) {
  if(alive) {
    if(!up && ((x == WIDTH) || (!page.getPixel(WIDTH - x, y)))) {
      uint8_t led_index = driver.getLedIndex24x5(WIDTH - x, y);
      driver.setPwmValue(0, led_index, random(32, 200));
    }
    page.setPixel(WIDTH - x, y, true);
  }
}
...
void Sparklie::initialiseSettings() {
  ...
  this->x = 0;
  this->y = 2;
  this->max_height = random(12, 24);
  this->max_displacement = random(0, 5);
